...
Voice messages are auto-saluted on dequeue (🫡). Replace with a different reaction only to convey additional meaning.
Processing preset (⏳) is ideal for audio: fires on dequeue, clears on send.
Priority queue: only the highest-priority reaction is visible; lower surfaces on expiry.
Temporary reactions auto-revert; permanent stay. Default temporality depends on the emoji.
