...
If your MCP client supports resource subscriptions, subscribe to `telegram://inbox/<token>` — TMCP pushes `notifications/resources/updated` on new messages and caps your `max_wait` to 90 s. Otherwise, if your MCP client supports file watching, call `activity/file/create` and watch the file; on change call dequeue(max_wait: 0). help('activity/file') and help('dequeue-http').
