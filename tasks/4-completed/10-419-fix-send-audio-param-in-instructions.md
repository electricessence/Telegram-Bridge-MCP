...
**Branch:** `10-419` in `Telegram MCP/.worktrees/10-419`
**Commit:** `88c4e06`

### What changed

- `.github/instructions/telegram-communication.instructions.md` — replaced `send(voice: ...)` with `send(audio: ...)` in Rule 9 (Non-Negotiable Rules, line 33) and the Tool Selection table "Conversational reply" row (line 48).

### Findings (Info only)

- **Info:** `npm warn Unknown project config "frozen-lockfile"` — pre-existing noise, unrelated to this change.
- **Info:** Rule 9 heading "Voice by default." intentionally left unchanged — describes UX concept, not API parameter name.
- **Info:** No other `.instructions.md` files contained `send(voice:` — no further changes needed.

### Verified

- All acceptance criteria met.
- Build passes (`npm run build` clean).
- `set_voice` references and nested `voice:` inside audio objects untouched (correct — different concept).
