...
Registered `chat/info` action path in `setupActionRegistry()` (`src/tools/action.ts`). `handleGetChat` was already imported; wired it to the new path. Two regression tests added: one verifies registration, one verifies dispatch routing. All 2131 tests pass. Branch: `10-426`, commit: `50678ab`.
