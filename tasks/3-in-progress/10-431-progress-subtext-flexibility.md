...
**Commit:** `be84fdf` on branch `10-431`

**Changes:**
- `src/tools/send_new_progress.ts`: `renderProgress()` subtext line changed from `` `<i>${escapeHtml(subtext)}</i>` `` to `escapeHtml(subtext)` — plain text, no wrap
- `src/tools/send_new_progress.test.ts`: 2 tests updated (italic→plain assertions), 1 regression test added
- `src/tools/update_progress.test.ts`: 1 test updated (italic→plain assertion)

**Build:** Clean · **Tests:** 2162/2162 passing
