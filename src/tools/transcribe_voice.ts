...
export function register(server: McpServer) {
  server.tool(
    "transcribe_voice",
    "Transcribes a Telegram voice message by its file_id. Use this when you have a voice message's file_id (returned in wait_for_message or get_updates voice results) but need to (re-)transcribe it — for example if the update was already consumed before transcription was read, or if transcription failed previously.",
    {
      file_id: z
        .string()
