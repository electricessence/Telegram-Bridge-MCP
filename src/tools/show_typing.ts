...
export function register(server: McpServer) {
  server.registerTool(
    "show_typing",
    {
      description: "Starts (or extends) a sustained background typing indicator that repeats every 4 s until the timeout expires or a real message is sent. Idempotent — safe to call multiple times; only one interval runs at a time. Cancelled automatically when any message is sent. Default timeout is 20 s.",
      inputSchema: {
        timeout_seconds: z
        .number()
        .int()
