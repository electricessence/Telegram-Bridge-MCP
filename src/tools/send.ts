          ...
          if (audio) {
            ...
            try {
              ...
              if (message_ids.length === 1) {
                return toResult({ message_id: message_ids[0], audio: true, ...(captionTruncated ? { info: "Caption was truncated to fit Telegram's 1024-character limit." } : {}), ...(usedMessageAlias ? { hint: "'message' is accepted as an alias. Canonical parameter: 'text'." } : {}) });
              }
              return toResult({ message_ids, split_count: message_ids.length, split: true, audio: true, ...(captionTruncated ? { info: "Caption was truncated to fit Telegram's 1024-character limit." } : {}), ...(usedMessageAlias ? { hint: "'message' is accepted as an alias. Canonical parameter: 'text'." } : {}) });
            } catch (err) {
