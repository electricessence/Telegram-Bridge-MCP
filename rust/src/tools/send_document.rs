//! send_document — sends a file/document to the configured chat.
...
pub struct SendDocumentToolParams {
...
pub async fn impl_send_document(params: SendDocumentToolParams) -> CallToolResult {
    ...
    let doc = FileUpload::String(params.document);
    let final_mode = if params.parse_mode == "HTML" { ParseMode::Html } else { ParseMode::MarkdownV2 };

    let mut doc_params = SendDocumentParams::builder()
    ...
    if let Some(cap) = params.caption {
        ...
        doc_params.caption = Some(cap);
    }
    if params.disable_notification == Some(true) { doc_params.disable_notification = Some(true); }
    if let Some(reply_id) = params.reply_to_message_id {
        doc_params.reply_parameters = Some(ReplyParameters::builder().message_id(reply_id).build());
    }

    match call_api(|| get_api().send_document(&doc_params)).await {
