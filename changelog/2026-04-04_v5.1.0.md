...
- `--http [port]` CLI flag — starts the Streamable HTTP server on the specified port (default 3099 if `--http` is passed without a value). Previously, HTTP mode required the `MCP_PORT` env var.
- `/approve` Telegram command — pre-authorizes the next `session_start` call to auto-approve without showing a color-picker dialog; useful for scripted session startup (e.g., spawn scripts that start sessions programmatically).
- Auto-approve integration — `session_start` now checks for a pending auto-approve token (via `checkAndConsumeAutoApprove()`) before showing the operator approval dialog; if a token is present the session is approved immediately without operator interaction.
...
- README Docker image tag updated from pinned `4.7.1` to `:latest` — prevents the README from going stale on future releases.
